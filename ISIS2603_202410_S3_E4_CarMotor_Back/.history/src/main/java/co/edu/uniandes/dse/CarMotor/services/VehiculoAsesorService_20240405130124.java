...
public class VehiculoAsesorService {
    ...
    @Transactional
    public List<AsesorEntity> getAsesores(List<VehiculoEntity> vehiculos) throws EntityNotFoundException {
        log.info("Inicia el proceso de todos los assesores");
        List<AsesorEntity> asesores = new ArrayList();
        for(long i = 0;i<VehiculoRepository.findAll().size();i++){
            if (!AsesorRepository.findAll().contains(VehiculoRepository.getById(i).getAsesor())) {
                throw new EntityNotFoundException(ErrorMessage.ASESOR_NOT_FOUND);
            }
            asesores.add(VehiculoRepository.getById(i).getAsesor());
        }
        log.info("Termina el proceso de conseguir todos los asesores = {0}");
        return asesores;
    }
