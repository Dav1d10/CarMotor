...
public class VehiculoAsesorService {
    ...
    public List<AsesorEntity> getAsesores() throws EntityNotFoundException {
        log.info("Inicia el proceso de asociarle un asesor al vehiculo con id = {0}");
        ...
        for(long i = 0;i<VehiculoRepository.findAll().size();i++){
            ...
            a.add(VehiculoRepository.getById(i).getAsesor());
        }
        vehiculoEntity.get().setAsesor(asesorEntity.get());
        log.info("Termina el proceso de asociarle un asesor al vehiculo con id = {0}");
        return a;
    }
